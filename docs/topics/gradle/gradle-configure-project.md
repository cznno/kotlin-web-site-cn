...
> visit the [Explore the build script](get-started-with-jvm-gradle-project.md#探索构建脚本) section.
>
{style="note"}
...
Kotlin 源代码可以与 Java 源代码放在相同目录或者不同目录中。
...
Now Gradle itself can run on any JDK and still reuse the [remote build cache feature](gradle-compilation-and-caches.md#gradle-构建缓存支持)
...
* Sets the [`compilerOptions.jvmTarget`](gradle-compiler-options.md#jvm-特有的属性) to the toolchain's JDK version
...
Projects targeting [multiple platforms](https://www.jetbrains.com/help/kotlin-multiplatform-dev/multiplatform-dsl-reference.html#目标), called [multiplatform projects](https://www.jetbrains.com/help/kotlin-multiplatform-dev/get-started.html),
...
different standard library versions. [Learn more about merging `kotlin-stdlib-jdk7` and `kotlin-stdlib-jdk8` into `kotlin-stdlib`](whatsnew18.md#更新了-jvm-编译项目标). 
