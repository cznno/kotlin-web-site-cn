...
It is divided into [operations that are the same](#在-java-与-kotlin-中相同的操作) 
and [operations that exist only in Kotlin](#java-标准库中不存在的操作). 
The second part of the guide, starting from [Mutability](#可变性), explains some of the differences by looking at specific cases.
...
| Check whether a collection contains an element or elements | `contains()`, `containsAll()` | Use the [`in` keyword](collection-elements.md#检测元素存在与否) to call `contains()` in the operator form: `element in collection`. |
...
| Get a stream from a collection | `stream()` | Kotlin has its own way to process streams: [sequences](#序列) and methods like [`map()`](collection-filtering.md) and [`filter()`](#过滤元素). |
...
| Check whether a map contains an element or elements | `containsKey()`, `containsValue()` | Use the [`in` keyword](collection-elements.md#检测元素存在与否) to call `contains()` in the operator form: `element in map`. |
...
| Apply the provided operation to collection elements sequentially and return the accumulated result | `stream().reduce()` | [`reduce()`, `fold()`](collection-aggregate.md#fold-与-reduce) |
...
| Take the first N elements | `stream().limit(N).collect()` | [`take(N)`](collection-parts.md#take-与-drop) |
| Take elements with a predicate | `stream().takeWhile().collect()` | [`takeWhile()`](collection-parts.md#take-与-drop) |
| Skip the first N elements | `stream().skip(N).collect()` | [`drop(N)`](collection-parts.md#take-与-drop) |
| Skip elements with a predicate | `stream().dropWhile().collect()` | [`dropWhile()`](collection-parts.md#take-与-drop) |
| Build maps from collection elements and certain values associated with them | `stream().collect(toMap(keyMapper, valueMapper))` | [`associate()`](collection-transformations.md#关联) |
...
See the [sequence processing example](sequences.md#序列处理示例), which shows the difference between `Iterable` and `Sequence`.
...
Learn more about [filtering maps](map-operations.md#过滤) here.
...
In Kotlin, use the [`zip()`](collection-transformations.md#合拢) function to do the same thing:
